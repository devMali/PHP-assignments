...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	try {
		if (empty($company)) {
			throw new Exception("Enter valid company");
		}

		if (empty($model)) {
			throw new Exception('Enter valid model');
		}

		$modelArr = explode('--', $model);

		if (strtolower($modelArr[0]) !== strtolower($company)) {
			throw new Exception("Selected company " . $company . ' has not model ' . $modelArr[1]);
		}
...
<body class='bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500'>
	...
	<div class='backdrop-blur-sm bg-white/50 w-96 mx-auto mt-16 rounded-md p-4'>
		<h2 class='text-lg font-medium mb-4'>Add Maintenance to file with exception handling:</h2>
	...
	if (isset($message) && !empty($message)) {
		echo "<div class='backdrop-blur-sm bg-white/50 w-96 flex items-center flex-col mx-auto mt-16 rounded-md p-4'>";
		echo $message;
		echo "</div>";
	}
	?>
